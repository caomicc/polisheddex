...
export function extractAllLocations(): Record<string, LocationData> {
  ...
  const mapAttributesPath = path.join(__dirname, 'polishedcrystal/data/maps/attributes.asm');
  ...
  console.log(
    `📋 Loaded canonical ordering: ${canonicalOrder.length} locations from landmark constants`,
  );
  ...
  for (const constantName of canonicalOrder) {
    if (constantName !== 'SPECIAL_MAP') {
      ...
      console.log(
        `constantName !== 'SPECIAL_MAP': ${normalizedKey}, id: ${id}, region: ${region} `,
      );
    ...
    if (landmarkMatch) {
      ...
      if (landmarkIndex < canonicalOrder.length) {
        ...
        if (landmarkConstantName && landmarkConstantName !== 'SPECIAL_MAP') {
          ...
          console.log(
            `Processing landmark: ${normalizedKey}, x: ${x}, y: ${y}, index: ${landmarkIndex}, region: ${
              (landmarkRegionMap[landmarkConstantName] || 'johto', landmarkConstantName)
            }`,
          );
  ...
  if (fs.existsSync(mapsDir)) {
    ...
    for (const mapFile of mapFiles) {
      ...
      console.log(`Processing map file: ${mapFile} for warps in ${sourceLocationKey}`);
      const mapPath = path.join(mapsDir, mapFile);
      const mapContent = fs.readFileSync(mapPath, 'utf8');
      const lines = mapContent.split(/\r?\n/);
      ...
      for (const line of lines) {
        ...
        if (warpMatch) {
          ...
          console.log(
            `🔍 Found warp from ${sourceLocationKey} (${path.basename(mapFile, '.asm')}) to ${targetLocationKey} (${targetMapConstant})`,
          );
      ...
      if (warpConnections.length > 0) {
        ...
        if (sourceLocationKey.includes('burned_tower')) {
          console.log(
            `🔗 Adding ${warpConnections.length} warp connections to ${sourceLocationKey}`,
          );
        }
        ...
        if (!locations[sourceLocationKey]) {
          ...
          const existingTargets = new Set(
            locations[sourceLocationKey].connections.map((c) => c.targetLocation),
          );
          const newConnections = warpConnections.filter(
            (c) => !existingTargets.has(c.targetLocation),
          );
  ...
  if (fs.existsSync(pokemonLocationPath)) {
    ...
    for (const pokemonLocationName of Object.keys(pokemonLocationData)) {
      ...
      if (!locations[normalizedPokemonKey]) {
        ...
        console.log(
          `📍 Created location entry for Pokemon location: ${pokemonLocationName} -> ${normalizedPokemonKey}`,
        );
      }
    }
  }
    ...
    if (locations[trainerKey]) {
      ...
      if (!matchedLocationKey) {
        for (const locationKey of Object.keys(locations)) {
          ...
          if (
            locationKey.includes('route') ||
            locationKey.includes('city') ||
            locationKey.includes('town')
          ) {
  ...
  const totalTrades = Object.values(locations).reduce(
    (sum, loc) => sum + (loc.npcTrades?.length || 0),
    0,
  );
  const totalEvents = Object.values(locations).reduce(
    (sum, loc) => sum + (loc.events?.length || 0),
    0,
  );
  const totalItems = Object.values(locations).reduce(
    (sum, loc) => sum + (loc.items?.length || 0),
    0,
  );
  const totalTMHMs = Object.values(locations).reduce(
    (sum, loc) => sum + (loc.tmhms?.length || 0),
    0,
  );
  const totalTrainers = Object.values(locations).reduce(
    (sum, loc) => sum + (loc.trainers?.length || 0),
    0,
  );
  const totalGymLeaders = Object.values(locations).reduce(
    (sum, loc) => sum + (loc.gymLeader ? 1 : 0),
    0,
  );
...
export async function exportAllLocations() {
  try {
    ...
    const groupsWithMultiples = Object.entries(consolidationGroups).filter(
      ([, keys]) => keys.length > 1,
    );
    if (groupsWithMultiples.length > 0) {
      ...
      const exampleKeys = Object.keys(consolidationGroups)
        .filter((key) => key.includes('burned'))
        .slice(0, 5);
    ...
    for (const [normalizedKey, originalKeys] of Object.entries(consolidationGroups)) {
      if (originalKeys.length === 1) {
        ...
        console.log(
          `   🔀 Consolidating ${originalKeys.length} entries for "${normalizedKey}": [${originalKeys.join(', ')}]`,
        );
        ...
        baseLocation.displayName =
          displayNames.find((name) => /[0-9]f?/i.test(name)) || displayNames[0];
        ...
        const uniqueConnections = allConnections.filter(
          (conn, index, self) =>
            index ===
            self.findIndex(
              (c) => c.direction === conn.direction && c.targetLocation === conn.targetLocation,
            ),
        );
        ...
        const uniqueEvents = allEvents.filter(
          (event, index, self) =>
            index ===
            self.findIndex((e) => e.type === event.type && e.description === event.description),
        );
        ...
        const uniqueTMHMs = allTMHMs.filter(
          (tmhm, index, self) =>
            index ===
            self.findIndex((t) => t.tmNumber === tmhm.tmNumber && t.moveName === tmhm.moveName),
        );
        ...
        const uniqueTrainers = allTrainers.filter(
          (trainer, index, self) =>
            index ===
            self.findIndex(
              (t) =>
    ...
    function createLogicalLocationOrder(): Map<string, number> {
      ...
      const locationGroups = [
        ...
        [
          'union_cave',
          'union_cave_1f',
          'union_cave_b1f_north',
          'union_cave_b1f_south',
          'union_cave_b2f',
        ],
        ['route_33'],
        [
        ...
        [
          ...
          'goldenrod_pokecom_center_2f_mobile',
        ...
        [
          'celadon_university',
          'celadon_university_1f',
          'celadon_university_2f_library',
          'celadon_university_2f_pool',
        ],
        [
          'saffron_city',
        ...
        [
          ...
          'fuchsia_mart',
          'fuchsia_gym',
          'fuchsia_bills_brothers_house',
          'fuchsia_safari_zone_office',
          'wardens_home',
        ],
        [
          'safari_zone',
     ...
     */
    function findParentLocationForGrouping(
      locationName: string,
    ...
    console.log(
      `✨ Consolidated ${Object.keys(allLocations).length} entries into ${Object.keys(consolidatedLocations).length} unique locations`,
    );
    ...
    console.log(
      `🔍 Filtered out region entries, ${Object.keys(filteredLocations).length} locations remaining`,
    );
    ...
    console.log(
      `📍 Exported ${orderedLocationsArray.length} ordered locations to ${orderedOutputPath}`,
    );
