...
export function EventCard(props: EventCardProps) {
  ...
  const iconImageUrl = (() => {
    ...
    if (variant === 'daily') {
      ...
      // if (deriveDailyType(event as any) === 'call') {
      //   return '/sprites/items/pokegear.png';
      // }
      switch (event.reward?.toLowerCase()) {
        case 'silk scarf':
          return '/sprites/items/scarf.png';
        default:
          return `/sprites/items/${event.reward?.toLowerCase().replace(/ /g, '_')}.png`;
      }
    }
    return '';
  })();
  ...
  return (
    ...
    >
      <div className="">
        ...
        {'reward' in event && event.reward ? (
          <span className="font-sans text-xs font-normal text-neutral-600 dark:text-neutral-300">
            <span className="font-bold">Reward:</span>{' '}
            <Link href={`/items/${event.reward?.toLowerCase().replace(/ /g, '')}`}>
              {event.reward}
            </Link>
          </span>
        ) : null}
        {'conditions' in event && event.conditions ? (
          <span className="inline-flex text-xs w-full flex-1 wrap-anywhere capitalize">
            {(event.conditions as string).replace(/_/g, ' ')}
          </span>
        ) : null}
      </div>
      {(('day' in event && event.day) ||
        ('days' in event && event.days && event.days.length > 0) ||
