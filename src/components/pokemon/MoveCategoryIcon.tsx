...
const MoveCategoryIcon: React.FC<MoveCategoryIconProps> = ({
  ...
  const normalizedCategory = String(category || "status").toLowerCase() as MoveDescription["category"];

  const icon = (
    <div className={cn("inline-flex items-center justify-center rounded-full w-8 h-8 p-2", `bg-${normalizedCategory}`, className)}>
      <Image
