...
const getTypesForMode = (data: DetailedStats, showFaithful: boolean, formName?: string): string[] => {
  let types;
  ...
  if (formName && data.forms && data.forms[formName]) {
    ...
    types = showFaithful 
      ? (data.faithfulTypes || data.types)
      : (data.updatedTypes || data.types);
  }
...
export function useTeamSearchParams(pokemonData: Record<string, DetailedStats>, showFaithful: boolean) {
  ...
  // Update team in URL
  const setTeam = useCallback((newTeam: (TeamPokemon | null)[]) => {
  ...
  const setPokemonInSlot = useCallback((index: number, name: string, data: DetailedStats, formName?: string) => {
    const newTeam = [...team];
    newTeam[index] = {
      name,
      formName,
  ...
  const removePokemonFromSlot = useCallback((index: number) => {
    const newTeam = [...team];
    newTeam[index] = null;
    setTeam(newTeam);
  }, [team, setTeam]);
